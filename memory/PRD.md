...
## Home/Browse performance audit (Aug 2026)
- Images: new src/components/CachedImage.tsx (expo-image) with blurhash placeholder (progressive), transition 200ms, cachePolicy memory-disk (no reload), recyclingKey, contentFit cover (downsamples to display size → not full-res in memory). Used for all remote images in home.tsx (surplus/featured/restaurant), browse-deals.tsx, surplus.tsx, restaurant/[id].tsx (hero + row). Header logo stays RN Image (require).
- Pagination/infinite scroll: backend /restaurants and /browse-deals accept limit & offset (default: no limit → returns all; preserves other callers). Sorting done then sliced. Frontend PAGE_SIZE home=12, browse=15; onEndReached appends next page, ListFooterComponent spinner, hasMore = last page length === PAGE_SIZE. FlatList perf props: initialNumToRender, maxToRenderPerBatch, windowSize, removeClippedSubviews.
- Non-blocking render: home loadData awaits ONLY the first restaurants page for the loading gate; drops + featured load in background via .then (a slow surplus/featured call no longer blocks the page). expo-image loads async so images never block.
- DB indexes added on startup: vendors.status, vendors(status,category), menu_items(vendor_id,available_today), menu_items(vendor_id,in_stock), menu_items.food_type, orders(vendor_id,status), orders(user_id,status). Verified created.
- Verified: /restaurants?limit&offset paginates correctly (page0≠page1), indexes present.

## Thumbnails + Skeletons + Cache freshness (Aug 2026)
- Thumbnails: backend _shrink_data_uri (Pillow) + _optimize_menu_images: on menu create (ops_add_menu_item) and update, base64 data-URI images are resized/recompressed to image_url (~1000px q78) and a thumbnail_url (~320px q60). Verified: 1500px 47KB → full 8KB + thumb 1KB. Exposed thumbnail_url via _menu_public, item_to_drop (dict spread), featured-deals (item_thumbnail), browse-deals (item_thumbnail). Frontend list views (home surplus/featured, browse, surplus screen) use CachedImage uri={thumbnail || full}; detail views keep full image_url. External http URLs left untouched. Existing images unchanged (applies to new create/update).
- Skeletons: src/components/Skeleton.tsx (SkeletonBox pulse + ListSkeleton variant restaurant/deal). Home: restaurant list ListEmptyComponent shows ListSkeleton while loading; surplus carousel shows deal skeleton while dropsLoading. Browse Deals + Surplus screens show restaurant-variant skeleton while loading (replaced spinners).
- Cache freshness: home uses useFocusEffect (skips first focus) → refreshSilently() re-fetches restaurants page 0 + drops + featured WITHOUT toggling the loading gate (no skeleton flash), so returning to Home shows newly posted deals.

## Menu Categories + Sorting + UI polish (Aug 2026)
- MENU CATEGORIES (free-form): menu_items gained a `menu_category` string (e.g. Starters, Mains, Drinks). Backend: OpsMenuItemBody + ops_add_menu_item/ops_update_menu_item persist it; exposed in _menu_public; Excel/CSV import (_normalize_menu_row + ops_bulk_add_menu) maps a "Menu Category"/"Section"/"Group" column. Ops MenuItemForm has a free-form "Menu Category" text field. Customer restaurant/[id].tsx GroupedMenu groups menu items under category headers (uncategorized -> "More" last); flat list when nothing categorized.
- SORTING: Restaurant menu has a sort bar (Recommended, Price Low->High, Price High->Low, Discount %); "Recommended" keeps category grouping, any sort flattens to a sorted list (distance omitted - single location). Surplus screen + Browse Deals have sort bars (Price Low->High [default], Price High->Low, Discount High->Low, Nearest). Backend added `price_desc` to /drops and /browse-deals sort_by.
- BROWSE ALL DEALS CARD: Home card redesigned - large branded green card (COLORS.primary bg), urgent-colored (accentUrgent) Tag icon, white title/subtitle + "Explore deals" CTA.
- ICONS/PLACEHOLDERS: new src/components/VegDot.tsx (green square + green dot, standard FSSAI veg symbol) replaces the Leaf icon in all Veg toggles (home/browse/surplus). CachedImage renders a branded "No image available" fallback (faded splash-icon on light-green bg, optional showLabel) whenever uri is empty; image-guard conditionals removed on restaurant MenuRow, home featured, browse-deals, surplus cards.
- Verified: testing_agent iteration_26 - backend 12/12 pytest (test_menu_category_and_sort.py) + full frontend E2E, no bugs.

## Multi-Item Cart (Aug 2026)
- CART STATE: new src/context/CartContext.tsx (AsyncStorage-persisted, key pg_cart_v1). A cart holds items from ONE vendor + ONE order mode (surplus/takeaway/dine_in). addItem() detects conflicts (different vendor OR different mode) and returns {conflict, vendorName, sameVendor}; replaceWithItem() clears + starts fresh. Provider wraps app in app/_layout.tsx.
- ADD-TO-CART: src/components/AddToCartSheet.tsx — bottom-sheet preview (image, name, price, qty stepper). On conflict it shows an in-sheet confirm ("Start a new cart? You already have items from [Restaurant]...") with Clear cart & Add / Cancel (custom Modal, not Alert, so it works on web + native). Triggered from restaurant menu rows and the surplus drop detail (drop/[id] "Add to Cart").
- VIEW CART BAR: src/components/CartBar.tsx — floating green bar (count + subtotal + vendor) shown on restaurant/[id] and drop/[id] when the cart has items; taps to /cart.
- CART SCREEN: app/cart.tsx — itemized list with qty steppers + remove + Clear, subtotal, Proceed to Checkout.
- CHECKOUT: app/checkout.tsx rewritten to read the cart (was single-item params). Shows item list + qty, pickup-slot selector, price breakdown (subtotal + 5% GST + 5% gateway fee), one Razorpay order for the whole cart. Guests -> /login?next=/checkout&resume=1 (cart persists), then auto-resumes payment. On success clears cart, passes items JSON to order-confirmation.
- BACKEND: POST /api/orders/create accepts items:[{food_item_id,quantity}] (legacy single still works). Validates empty (400), same-vendor-only (400), active + open, per-item surplus qty; pending_orders stores the items array + summed totals. _finalize_order builds order.items[], summary food_item_name ("First + N more"), summed quantity + item_subtotal; decrements surplus qty per item. cancel restores qty per item. reorder returns vendorId. GET /orders/user, /vendor/orders, ops orders return the items array.
- RECEIPTS/LISTS: order-confirmation, My Orders (orders.tsx), and Vendor dashboard (dashboard.tsx) render the itemized list for multi-item orders (single-item unchanged). "Order again" routes through the cart (replaceWithItem -> /cart) per user choice.
- Verified: testing_agent iteration_27 backend 13/13 pytest (test_multi_item_cart.py) + frontend cart/checkout flows; cross-restaurant conflict Modal verified via screenshot. Razorpay is LIVE so payment completion was not executed.

## Performance pass 2 — Grow tier (Aug 2026)
- Lazy loading: PAGE_SIZE lowered to 10 for Nearby Restaurants (home) and Browse Deals (infinite scroll already in place; backend /restaurants & /browse-deals paginate via limit/offset).
- Memoization: extracted RestaurantCard (home) and DealCard (browse-deals) into React.memo components with a stable useCallback onPress (openRestaurant) so cards don't re-render while scrolling.
- DB indexes added: vendors.category (single), menu_items.available_today (single, = isSurplus), menu_items(available_today,in_stock). Existing: vendors.status, (status,category), menu_items(vendor_id,available_today), (vendor_id,in_stock), food_type, orders(vendor_id,status)/(user_id,status). NOTE: vendor location is stored as {lat,lon} and distance is computed in-app (haversine) — queries don't do geo filtering, so no 2dsphere index is used.
- Images already optimized earlier: CachedImage (expo-image, memory-disk cache, blurhash progressive), list views request thumbnail_url (~320px) not full-res; backend generates thumbnails on menu create/update.

## Perf pass 3 + Object Storage (Aug 2026)
- versionCode bumped 110 -> 136 in frontend/app.json (Play Store 'version already in use' fix). version stays 1.0.2.
- Prefetch: onEndReachedThreshold raised 0.5 -> 1.5 on home Nearby Restaurants + browse-deals so the next 10 load before hitting the bottom.
- Home cart access: CartBar now renders on the Home tab (bottomOffset=64 to clear the tab bar) so shoppers keep quick access to their cart.
- IMAGE OBJECT STORAGE (Emergent managed): NEW image uploads now go to object storage instead of base64-in-DB. Backend helpers _init_storage/_storage_put/_storage_get (init at startup, 503 stale-key retry), _store_image_variants(data_uri, owner) uploads full(~1000px q78) + thumb(~320px q60) JPEGs and returns /api/files/{path} URLs (falls back to inline base64 if storage down). Public read route GET /api/files/{path:path} serves bytes with immutable Cache-Control. Wired into ops_add_menu_item, ops_update_menu_item, _bulk_update_images (zip), ops_create_vendor + ops_update_vendor (storefront_image). Uses EMERGENT_LLM_KEY + INTEGRATION_PROXY_URL. APP_NAME='perfectly-good', path {app}/uploads/{owner}/{uuid}.jpg. EXISTING base64 images left as-is (both coexist).
- Frontend: client.ts exports resolveMediaUrl()/API_ORIGIN — prefixes relative /api/... media URLs with the backend origin. CachedImage resolves via it; raw <Image> sources also wrapped (drop hero, dashboard menu thumb, onboarding deal, ops vendor detail, ops MenuItemForm/VendorForm previews). Restaurant hero + list cards use CachedImage.
- Verified: object-storage upload->serve tested end-to-end (POST menu item returns /api/files URLs; GET serves image/jpeg w/ immutable cache) + screenshot shows storage image, legacy base64, and branded placeholder coexisting. NOTE: Emergent object storage has no public CDN URL — reads proxy through /api/files (billed per read); acceptable per user's explicit choice.

## Perf pass 4 + Cart Notes + Vendor Thumbnails (Aug 2026)
- CART NOTES: CartItem gained optional `note`; AddToCartSheet has a note TextInput (e.g. "no onions"), cart screen shows an editable note per item (updateNote), checkout sends note per line item. Backend create_order stores `note` per priced item; _finalize_order carries it; vendor dashboard order card shows "↳ note" under each item; checkout summary shows "Note: ...".
- VENDOR THUMBNAILS: ops_create/update_vendor now store storefront_thumbnail (~320px) alongside storefront_image (full) via _store_image_variants; exposed in _vendor_public + _vendor_card; home RestaurantCard prefers storefront_thumbnail so the list loads lighter.
- HOME INSTANT LOAD (Task A): home caches the default restaurant list in AsyncStorage (pg_home_restaurants_v1). On open, cached list renders immediately (no skeleton if cache present) while loadData() fetches fresh in the background; refreshSilently also re-persists. Cache only for the default (no search/category) view.
- SLIM PAYLOAD (Task B): new _vendor_card() serializer used by GET /restaurants returns only card fields (vendor_id, name, category, storefront_thumbnail, logo_url, discount_percentage, is_open, verified) + menu_count/surplus_count/has_veg/distance. Dropped hours, today_shifts, full location, full-res storefront, pickup times from the list. /restaurants/{id} still returns full _vendor_public.
- GEOSPATIAL (Task C): vendors now store GeoJSON `location_geo` (Point [lon,lat]); populated on create/update/profile; startup backfills existing + creates a 2dsphere index. GET /restaurants uses $geoNear (spherical, key=location_geo) for nearest-first when lat/lon provided, appends active vendors lacking geo, and falls back to haversine on error. (Test vendors have 0/0 coords so distance shows null until real coords geocoded.)
- CDN CACHE (Task D): /api/files already serves images with Cache-Control public, max-age=31536000, immutable (>7 days); expo-image caches memory+disk. Legacy base64 images are inline (cached with the JSON). No change needed.
